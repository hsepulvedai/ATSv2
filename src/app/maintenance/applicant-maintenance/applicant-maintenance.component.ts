...
export class ApplicantMaintenanceComponent implements OnInit {
  ...
  inactiveFilteredApplicants: IApplicant[]
  ...
  updatedApplicant: IApplicant

  allApplicants: IApplicant[]
  allPastApplicants: IApplicant[]
  ...
  performFilter(filterBy: string): IApplicant[] {
    filterBy = filterBy.toLocaleLowerCase();


    if (this.selectedFilter === 'Name/FirstName')
      return this.allApplicants.filter((applicant: IApplicant) =>
  ...
  performInactiveFilter(filterBy: string): IApplicant[] {
    filterBy = filterBy.toLocaleLowerCase();


    if (this.selectedFilter === 'Name/FirstName')
      return this.allPastApplicants.filter((applicant: IApplicant) =>
        applicant.firstName.toLocaleLowerCase().indexOf(filterBy) !== -1);
  ...
  refreshData() {
   ...
   sortedData: IApplicant[]
   sortedInactive: IApplicant[]
